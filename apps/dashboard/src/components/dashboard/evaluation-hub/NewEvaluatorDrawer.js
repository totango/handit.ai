...
import { X, Plus, ArrowsOutSimple, Code, Check, PencilSimple } from '@phosphor-icons/react/dist/ssr';
...
export default function NewEvaluatorDrawer({ open, onClose, onCreate, associateSection = true }) {
  ...
  // Reset all state when drawer is closed or after creation
  useEffect(() => {
    if (!open) {
      setName('');
      setPrompt('');
  ...
  const handleAssociationChange = (modelId, field, value) => {
  ...
  return (
    <Drawer
      ...
      open={open}
      onClose={onClose}
      PaperProps={{ sx: { width: 500, p: 3, bgcolor: 'background.default' } }}
    >
                    ...
                    <Stack direction="row" alignItems="center" justifyContent="space-between">
                      ...
                      <Stack direction="row" spacing={1}>
                        ...
                        >
                          <MenuItem
                            onClick={() => {
                              handleRemoveAssociation(assoc.modelId);
                              setAssocMenuAnchorEl(null);
                              setAssocMenuModelId(null);
                            }}
